...
export function expandEventsForView(baseEvents: any[], viewStartDate: Date, viewEndDate: Date) {
  ...
  baseEvents.forEach(event => {
    ...
    try {
      ...
      const generatedDates = rule.between(viewStartDate, viewEndDate, true);
      ...
      generatedDates.forEach(date => {
        ...
        const exceptionOverride = baseEvents.find(e => 
        ...
        if (exceptionOverride) {
          ...
          if (event.endDate && event.endDate !== event.date) {
            ...
            const diffDays = Math.round((endDt.getTime() - startDt.getTime()) / (1000 * 3600 * 24));
