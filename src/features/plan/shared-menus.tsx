...
export function FilterSortButtons({ filters, setFilters, sort, setSort, columns, getValuesForColumn, sortOptions, align = 'left' }) {
  ...
  const addFilter = () => {
    ...
    setFilters([...filters, { id: Date.now().toString(), column: firstCol, operator: 'is', value: firstVal }]);
  };
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', gap: '8px', alignItems: 'center' }} ref={ref}>
      <button 
      ...
      {showFilters && (
        <div className={`floating-menu ${closingFilters ? 'is-closing' : ''}`} style={{ 
          position: 'absolute', 
          top: 'calc(100% + 8px)', 
      ...
      {showSort && sortOptions && (
        <div className={`floating-menu ${closingSort ? 'is-closing' : ''}`} style={{ 
          position: 'absolute', 
          top: 'calc(100% + 8px)', 
          left: align === 'left' ? 0 : 'auto',
          right: align === 'right' ? 0 : 'auto', 
