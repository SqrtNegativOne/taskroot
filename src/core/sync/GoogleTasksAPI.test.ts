...
describe('GoogleTasksAPI', () => {
  ...
  describe('fetchTasks', () => {
    ...
    it('throws Unauthorized on 401', async () => {
      ...
      await expect(googleTasksAPI.fetchTasks()).rejects.toThrow('Unauthorized');
    });
  });

  describe('toGoogleTask', () => {
    it('prepends Taskroot Task ID to notes if not present', () => {
      const localTask = { id: 't123', title: 'Buy milk', status: 'todo', notes: '2 percent' };
    ...
    it('does not prepend if already present', () => {
      const localTask = { id: 't123', title: 'Buy milk', status: 'done', notes: 'Taskroot Task ID: t123\n2 percent' };
      const googleTask = googleTasksAPI.toGoogleTask(localTask);
  ...
  describe('toLocalTask', () => {
    ...
    it('handles deleted tasks', () => {
      const googleTask = { id: 'g123', deleted: true, updated: '2023-01-01T00:00:00Z' };
      const localTask = googleTasksAPI.toLocalTask(googleTask, { id: 't456' });
