...
describe("SyncEngine", () => {
    ...
    it("polls tasks and resolves conflicts by taking the latest updatedAt", async () => {
        // Setup local task
        const localTasks = [{ id: "t1", title: "Local Task", updatedAt: 1000 }];
    ...
    it("queues updates when local data changes", async () => {
        ...
        expect((googleTasksAPI as any).fakeTasks.get("g1").title).toBe(
            "Task 1 Edited",
        );
    });
    ...
    it("queues creates when local data is new", async () => {
        ...
        const newTasks = [{ id: "t2", title: "New Task", updatedAt: 1000 }];
        ...
        const createdRemoteTask = (googleTasksAPI as any).fakeTasks.get(
            saved[0].googleTaskId,
        );
