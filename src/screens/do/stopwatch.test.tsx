...
describe('CounterClockStrategy', () => {
  ...
  test('requiresAnimationLoop when running', () => {
    ...
    expect(strategy.requiresAnimationLoop({ state: { runningSince: null } })).toBe(false);
  });

  test('onToggle toggles state', () => {
    const setState = vi.fn(updater => updater({ elapsed: 0, runningSince: null }));
    ...
    // Stop
    const setState2 = vi.fn(updater => updater({ elapsed: 1000, runningSince: 1000 }));
    const setTimeLogs = vi.fn();
    strategy.onToggle({
      isPristine: false,
      setSelectorOpen,
...
describe('FlowtimeClockStrategy', () => {
  ...
  test('onToggle prevents pause during break', () => {
    const setState = vi.fn(updater => updater({ elapsed: 100, runningSince: null, isBreak: true }));
    strategy.onToggle({
      isPristine: false,
