...
import { DEFAULT_STATUSES, DEFAULT_DISTRACTION_COLUMNS, SAMPLE_DISTRACTIONS, MONTHS, PAD2 } from '../../core/store/data';
...
export function DistractionLog() {
  ...
  const addRow = () => {
    ...
    const id = `d${Date.now()}`;
    setRows(rs => [...rs, { id, name: '', status: statuses[0]?.id || '', created }]);
  ...
  const resizeCol = (colId, startWidth, startX) => (e) => {
    ...
    const onMove = (ev) => {
      ...
      setColumns(cs => cs.map(c => c.id === colId ? { ...c, width: newW } : c));
    };
    const onUp = () => {
  ...
  const addStatus = (label, color) => {
    if (!label) return;
    const id = label.toLowerCase().replace(/\s+/g, '-') + '-' + Date.now().toString(36).slice(-4);
  ...
  return (
    <div className="dlog">
      <div className="dlog-scroll">
        <div className="dlog-table" style={{ minWidth: totalWidth }}>
          ...
          <div className="dlog-row dlog-row-head">
            {columns.map(col => (
              <div key={col.id} className="dlog-cell dlog-cell-head" style={{ width: col.width }}>
                <span className="dlog-cell-head-icon">{colIcon(col.type)}</span>
                <span className="dlog-cell-head-label">{col.label}</span>
                <span
                  className="dlog-col-resize"
                  onPointerDown={(e) => resizeCol(col.id, col.width, e.clientX)(e)}
          ...
          {rows.length === 0 && (
            <div className="dlog-row dlog-row-empty">
              <span className="dim">no distractions logged. nice.</span>
            </div>
          )}
          {rows.map(row => (
            <DLogRow
      ...
      <div className="dlog-foot">
        <span className="dim">{rows.length} {rows.length === 1 ? 'row' : 'rows'}</span>
        <span className="dlog-foot-sep">·</span>
        {statuses.map(s => (
          <span key={s.id} className="dlog-foot-stat">
            <span className="status-chip" style={{ background: hexAlpha(s.color, 0.18), color: s.color, borderColor: hexAlpha(s.color, 0.6) }}>
              {s.label}
            </span>
            <span className="dim"> {rows.filter(r => r.status === s.id).length}</span>
          </span>
        ))}
...
function DLogRow({ row, columns, statuses, editingCell, setEditingCell, statusEditor, setStatusEditor, updateRow, deleteRow, addStatus }) {
  return (
      ...
      {columns.map(col => {
        ...
        return (
          <div
            ...
            onClick={() => {
              if (col.type === 'text') setEditingCell({ rowId: row.id, colId: col.id });
              else if (col.type === 'status') setStatusEditor(row.id);
            }}
          >
            {col.type === 'text' && (
              isEditing
            ...
            {col.type === 'status' && (
              <StatusCell
                ...
                onChange={(v) => { updateRow(row.id, { [col.id]: v }); setStatusEditor(null); }}
                onAdd={(label, color) => {
            ...
            {col.type === 'datetime' && (
              <span className="dlog-datetime">{formatDateTime(row[col.id])}</span>
            )}
          </div>
        );
      })}
      <div className="dlog-cell dlog-cell-actions">
        <button className="dlog-row-x" onClick={(e) => { e.stopPropagation(); deleteRow(row.id); }} title="Delete row">×</button>
      </div>
    </div>
  );
}

function StatusCell({ value, statuses, open, onOpen, onClose, onChange, onAdd }) {
  ...
  return (
    <>
      <span
        ...
        style={current ? { background: hexAlpha(current.color, 0.18), color: current.color, borderColor: hexAlpha(current.color, 0.6) } : { color: 'var(--fg-dim)' }}
      >
        {current ? current.label : <span className="dim">select…</span>}
      </span>
      {open && (
        <div className="dlog-status-pop" ref={popRef} onClick={(e) => e.stopPropagation()}>
          {statuses.map(s => (
            <button key={s.id} className="dlog-status-opt" onClick={() => onChange(s.id)}>
              <span className="status-chip" style={{ background: hexAlpha(s.color, 0.18), color: s.color, borderColor: hexAlpha(s.color, 0.6) }}>
                {s.label}
              </span>
            </button>
          ...
          {addingStatus ? (
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter') {
                    ...
                    setNewLabel(''); setAddingStatus(false);
                  } else if (e.key === 'Escape') {
                    setAddingStatus(false); setNewLabel('');
                  }
                }}
              />
              <div className="dlog-status-colors">
                {['var(--tag-red)','var(--tag-yellow)','var(--tag-green)','var(--tag-purple)','var(--tag-gold)','#9bb0d4'].map(c => (
                  <button
                    ...
                    style={{ background: hexAlpha(c, 0.22), borderColor: c, color: c }}
            ...
            <button className="dlog-status-opt dlog-status-add" onClick={() => setAddingStatus(true)}>
