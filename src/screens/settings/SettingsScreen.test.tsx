...
describe('SettingsScreen', () => {
  ...
  it('filters settings when searching', () => {
    ...
    act(() => {
      fireEvent.change(searchInput, { target: { value: firstSetting.label } });
    });
    ...
    expect(screen.queryByText(SETTINGS_TABS[0].label)).not.toBeInTheDocument();
  });
  ...
  it('renders clear all data button in danger zone', () => {
    ...
    const searchInput = container.querySelector('.search-input') as HTMLInputElement;
    act(() => {
      fireEvent.change(searchInput, { target: { value: 'Clear All Data' } });
    });
