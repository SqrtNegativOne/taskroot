    ...
    React.useEffect(() => {
        ...
        if (validTasks.length !== tasks.length) {
            ...
            setEvents((es) =>
                es.filter((e) => {
                    if (e.taskId)
                        return validTasks.some((t) => t.id === e.taskId);
...
function InspectorPane({
    ...
    const [calendars] = useStored("calendars", [
    ...
    const currentItem = currentState
        ? isCurrentTask
            ...
            : events.find((e) => e.id === currentState.id) ||
              events.find((e) => e.id === currentState.id.split("_")[0])
        : null;
    ...
    React.useEffect(() => {
        if (
            ...
            currentItem?.endDate &&
            currentItem?.date &&
            currentItem.endDate !== currentItem.date
        ) {
            setShowEndDate(true);
        } else {
    ...
    const handleClose = React.useCallback(() => {
        ...
        if (isCurrentTask) {
            setTasks((ts) => {
                ...
                if (t && t.isDraft) {
                    setEvents((es) =>
                        es.filter((e) => e.taskId !== currentItem.id),
                    );
            ...
            setEvents((es) => {
                ...
                if (e && e.isDraft)
                    return es.filter((x) => x.id !== currentItem.id);
                return es;
            });
        }
    ...
    const title = currentItem
        ? isCurrentTask
            ? currentItem.title
            : currentItem.taskId
              ? tasks.find((t) => t.id === currentItem.taskId)?.title
              : currentItem.title
        : "";
